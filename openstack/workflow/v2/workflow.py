...
class Workflow(resource2.Resource):
    ...
    base_path = '/queues/%(queue_name)s/workflows'
    ...
    # Properties
    #: The value in second to specify how long the workflow has been
    #: posted to the queue.
    age = resource2.Body("age")
    #: A dictionary specifies an arbitrary document that constitutes the
    #: body of the workflow being sent.
    body = resource2.Body("body")
    #: An uri string describe the location of the workflow resource.
    href = resource2.Body("href")
    #: The value in seconds to specify how long the server waits before
    #: marking the workflow as expired and removing it from the queue.
    ttl = resource2.Body("ttl")
    #: The name of target queue workflow is post to or got from.
    ...
    def post(self, session, workflows):
        ...
        request.body = {'workflows': workflows}
    ...
    def list(cls, session, paginated=True, **params):
        """This method is a generator which yields workflow objects.
    ...
    def delete(self, session):
        ...
        # parameter when deleting a workflow that has been claimed, we
