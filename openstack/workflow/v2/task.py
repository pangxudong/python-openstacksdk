...
class Task(resource2.Resource):
    ...
    workflow_execution_id = resource2.Body("workflow_execution_id")
    state = resource2.Body("state")
    state_info = resource2.Body("state_info")
    result = resource2.Body("result")
    published = resource2.Body("published")
    processed = resource2.Body("processed")
    reset = resource2.Body("reset")
    env = resource2.Body("env")
